...
for split in ['train', 'val', 'test']:
    ...
    tmp = []
    for inp, bs, dbsearch, act, trg in zip(history_raw_new, belief_raw_new, db_search_raw, action_raw_new, output_raw_new):
        tmp.append(' '.join([inp.lower(), bs.lower(), dbsearch.lower(), act, trg]))
    with open('{}/{}.history_belief_dbsearch_action_sys'.format(save_dir, split), 'wt') as f:
        for l in tmp:
